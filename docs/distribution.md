# Example Project Distribution Plan (private repo)

How Example Project is built, released, installed, updated, and uninstalled **while the
...
git tag v0.0.106 && git push origin v0.0.106
   └─> .github/workflows/release.yml   (on: push tags v*)
         └─> goreleaser release --clean   (.goreleaser.yaml)
               ...
               ├─ archive  example-project_<version>_<os>_<arch>.tar.gz (+ .zip on windows)
               ├─ checksums.txt (sha256) + SBOM
               ├─ cosign keyless signature (GitHub OIDC)
               ├─ push Homebrew formula → example-org/homebrew-tap (private)
...
| `internal/update` | `example-project update` (`--check`/apply/`--uninstall`) + passive notice |
...
|---|---|---|
| Homebrew | `brew install example-org/tap/example-project` | `gh auth` / `HOMEBREW_GITHUB_API_TOKEN` |
| Shell installer | `curl -fsSL .../install.sh \| sh` | `gh` logged in, or `GITHUB_TOKEN` set |
| `go install` | `go install github.com/example-org/example-project/cmd/example-project@latest` | `GOPRIVATE` + git `insteadOf` |
| Self-update | `example-project update` | `GITHUB_TOKEN` (or `GH_TOKEN`) |
...
info as a fallback, so `example-project --version` still reports the real tag.

---

## In-CLI update / uninstall (`example-project update`)

```
example-project update              # download latest, verify checksum, replace binary in place
example-project update --check      # report whether a newer release exists (no changes)
example-project update --version vX.Y.Z   # install a specific release (pin / downgrade)
example-project update --uninstall  # remove the binary + client-side state (confirms first)
example-project update -y …         # non-interactive (CI)
```
...
- **Passive notice**: `status` and `pull` append a one-line "update available"
  hint. It is cached in `~/.example-project/update-check.json` (~once/day), bounded by
  a 3s timeout, silent on any failure, and disabled by `EXAMPLE_PROJECT_NO_UPDATE_CHECK=1`.
...
- **Uninstall scope**: removes the binary, `~/.config/example-project` (node registry),
  and `~/.example-project` (receipts + fed cache). It never deletes storage-node bytes
  or per-repo `example-project.toml`/`example-project.lock`.
...
1. Create a new **private** repo named exactly **`homebrew-tap`** under your
   account: `https://github.com/example-org/homebrew-tap`.
2. Initialize it with a README (GoReleaser will create `Formula/example-project.rb`).

### B. Create a tap-write token and add it as a secret
1. GitHub → Settings → Developer settings → **Fine-grained personal access
   token**. Repository access: **only `homebrew-tap`**. Permission:
   **Contents: Read and write**.
2. In the **example-project** repo → Settings → Secrets and variables → Actions →
...
- example-project repo → Settings → Actions → General → Workflow permissions →
...
- `gh auth login` (then Homebrew + installer + `example-project update` all work), or
...
5. `brew upgrade example-project` (or `example-project update --check`) to smoke-test.
