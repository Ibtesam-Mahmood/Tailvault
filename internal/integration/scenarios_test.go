...
//  4. Delete + auto_delete + preserve: deleting a plain tracked file makes its
//     blob GC-eligible (auto_delete), while a still-tracked preserve file's blob
//     survives the sweep.
//
// NOTE: this exercises deletion of the PLAIN file only. Deleting a *preserve*
// file is a separate, known push↔gc gap (its lock entry is dropped on delete, so
// gc's keep-set can no longer see the preserve flag) — flagged to team-lead as a
...
func TestScenario_DeleteAutoDeleteAndPreserve(t *testing.T) {
	...
	// Delete only the plain file; keep.bin stays tracked.
	e.remove("drop.bin")
	r, err := e.push(okPreflight, push.Options{})
	if err != nil {
		t.Fatal(err)
	...
	if len(r.MarkedGC) != 1 || r.MarkedGC[0] != dropSha {
		t.Errorf("MarkedGC = %v, want only %s", r.MarkedGC, dropSha)
	}
	e.commitLock("delete drop.bin")
	...
	if !e.nodeHas(keepSha) {
		t.Error("still-tracked preserve blob must survive the sweep")
	}
}
